/**
 * 123pan 自动鉴权客户端 (Auto Mode) - 配合 Cloudflare Workers 使用
 * 
 * worker 代码见： https://github.com/hcllmsx/Authn123Pan/For-cloudflareWorkers/worker.js
 * 作者：https://github.com/hcllmsx
 * 
 * 
 * 使用方法：
 * 1. 在HTML中引入此脚本
 * 2. 配置 Worker URL（通过 data-worker-url 属性或全局配置）
 * 3. 在视频/图片元素上添加 data-123pan-src 属性，值为123pan的原始URL
 * 4. 脚本会自动处理鉴权，只在用户操作时才请求签名
 * 
 * 示例：
 * <script src="auth-client-auto.js" data-worker-url="https://your-worker.workers.dev"></script>
 * 
...
(function (window, document) {
    ...
    function autoConfig() {
        ...
        if (scriptTag) {
            const workerUrl = scriptTag.dataset.workerUrl || scriptTag.getAttribute('data-worker-url');
        ...
        // 如果启用了自动初始化
        if (Pan123Auth.config.autoInit && Pan123Auth.config.workerUrl) {
